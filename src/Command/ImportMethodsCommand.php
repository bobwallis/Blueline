...
class ImportMethodsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        pg_upsert($db, 'collections', array('name' => 'Plain Minor Methods', 'description' => 'All the possible symmetric Bob and Place Minor methods with five leads in the plain course.'), array('id' => 'pmm'));
        // and the 'Treble Dodging Minor Method' collection
        pg_upsert($db, 'collections', array('name' => 'Treble Dodging Minor Methods', 'description' => 'All the possible symmetric Treble Bob, Delight and Surprise Minor methods with five leads in the plain course and with no bell making more than two consecutive blows in the same position.'), array('id' => 'tdmm'));
        ...
        foreach ($dataFiles as $file) {
            ...
            foreach ($xmlIterator as $xmlRow) {
                ...
                if (pg_upsert($db, 'methods', array_intersect_key($xmlRow, $validFields), array('title' => $xmlRow['title'])) === false) {
                    if (pg_delete($db, 'methods', array('title' => $xmlRow['title'], 'provisional' => true))
                         && pg_upsert($db, 'methods', array_intersect_key($xmlRow, $validFields), array('title' => $xmlRow['title']))) {
                        $progress->clear();
