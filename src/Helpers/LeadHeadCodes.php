...
class LeadHeadCodes
{
    ...
    private static $_leadHeadCodeConversion = [
        'a' => 'g',
        'b' => 'h',
        'c' => 'j',
        'c1' => 'j1',
    ...
    public static function toCode($leadHead, $stage, $leadEndNotation, $postLeadEndNotation = '')
    {
        ...
        if ($code) {
            $isPQFamily = 'p' === $code[0] || 'q' === $code[0];
            if ($stageIsEven) {
                if ((!$isPQFamily && '12' == $leadEndNotation) || ($isPQFamily && $postLeadEndNotation == '3'.$stageNotation)) {
                    return $code;
                }
                if ((!$isPQFamily && $leadEndNotation == '1'.$stageNotation) || ($isPQFamily && 'x' == $postLeadEndNotation)) {
                    return str_replace(array_keys(self::$_leadHeadCodeConversion), array_values(self::$_leadHeadCodeConversion), $code);
                }
            } else {
                if (($isPQFamily && $leadEndNotation == '12'.$stageNotation) || (!$isPQFamily && '3' == $postLeadEndNotation)) {
                    return $code;
                }
                if (($isPQFamily && '1' == $leadEndNotation) || (!$isPQFamily && $postLeadEndNotation == $stageNotation)) {
                    return str_replace(array_keys(self::$_leadHeadCodeConversion), array_values(self::$_leadHeadCodeConversion), $code);
                }
            }
        }
