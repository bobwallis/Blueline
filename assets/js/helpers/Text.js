...
var core = [
	...
	CorePlaceMultipleFixFunction = function (match, p1, p2) {
		...
		if (count > 2) {
			return match.replace(new RegExp(p1, 'g'), `${count} blows in ${p2}ths,`);
		} else {
			return match;
		}
	};
...
var Text = {
	...
	fromRows: function (rows, bell, wrap) {
		...
		for (i = 0; i < coreRows.length; ++i) {
			...
			while (i < coreRows.length && coreRows[i] === coreRows[i + 1]) {
				++i;
			}
		}
		coreTextHuman = coreTextHuman.replace(trimtrailingcommaregex, '');
		coreTextHuman = coreTextHuman.toLowerCase()
			.replace(/(^|\. )(.)/g, function (m, p1, p2) {
				return p1 + p2.toUpperCase();
			})
			.replace(/\.\s*$/, '')
			.replace(/^\. (h|H)/, 'H');
		coreTextHuman = `${coreTextHuman}.`;
