...
var GridOptionsBuilder = function (options) {
	...
	if (typeof this.ruleOffs.show == 'undefined') {
		this.ruleOffs.show = true;
	}
	if (typeof options.callingPositions === 'object') {
		this.callingPositions = options.callingPositions;
		if (typeof this.callingPositions.show == 'undefined') {
			this.callingPositions.show = true;
		}
		if (typeof this.callingPositions.font == 'undefined') {
			this.callingPositions.font = `${fontSize * 0.8}"Lucida Grande", "Lucida Sans Unicode", "Lucida Sans", Geneva, Verdana, sans-serif`;
		}
		if (typeof options.workingBell == 'string' && options.workingBell == 'lightest') {
			this.callingPositions.show = false;
		}
	} else {
	...
	var placeStarts = toFollow.filter(function (b) {
	...
	if (typeof options.calls === 'object') {
		for (var callTitle in options.calls) {
			if (Object.prototype.hasOwnProperty.call(options.calls, callTitle)) {
				...
				while (notationExploded.length < (2 * call.every) + call.from + call.cover) {
	...
	this.gridOptions.calls.numbers = function () {
		...
		sharedCallsGridOptions.forEach(function (call, callIndex) {
			...
			thisgridOptionscallsnumbers[callIndex].numbers = { show: true, font: font, bells: rounds.map(function (b) {
				return { color: (thisgridOptionscallsnumbers[callIndex].affected.indexOf(b) !== -1 || this.huntBells.indexOf(b) !== -1) ? 'transparent' : '#000' };
			}, that) };
			// Set line colors
	...
	this.gridOptions.calls.diagrams = function () {
		...
		sharedCallsGridOptions.forEach(function (call, callIndex) {
			...
			thisgridOptionscallsdiagrams[callIndex].numbers = { show: true, font: `${fontSize * 0.9}px ${fontFace}`, bells: rounds.map(function (b) {
				return { color: '#002856' };
			}) };
			// Set line colors
			for (i = 0, j = 0; i < that.stage; ++i) {
