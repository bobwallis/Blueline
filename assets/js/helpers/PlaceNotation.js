...
var PlaceNotation = {
	...
	expand: function (notation, stage) {
		...
		if (typeof stage === 'undefined') {
			stage = Math.max.apply(Math, notation.replace(/( HL |LH|LE)/g, '').split('').map(PlaceNotation.charToBell).filter(function (b) {
		...
		if (fullNotation.indexOf(',') !== -1) {
			var splitOnComma = fullNotation.split(',').map(function (s) {
				return s.trim();
			});
		...
		fullNotation = PlaceNotation.explode(fullNotation).map(function (piece) {
			...
			var affected = piece.match(/[A-Z\d]+/g).join('').split('').map(PlaceNotation.charToBell).sort(function (a, b) {
				return a - b;
			});
					...
					.map(function (e) {
						if (e.charAt(0) == '(' || e.charAt(0) == '[') {
							...
							return e.split('').map(function (f) {
								return { sort: PlaceNotation.charToBell(f), value: f };
							});
						}
					}))
				.sort(function (a, b) {
	...
	parse: function (notation, stage) {
		...
		for (i = 0; i < stage; i += 2) {
			xPermutation[i] = i + 1; xPermutation[i + 1] = i;
		}
		if (i - 1 === stage) {
	...
	queens: function (stage) {
		if (stage < 4) {
			throw new Error('Stage must be >= 4 for Queens notation.');
		}
		if (stage % 2 === 1) {
			return PlaceNotation.queens(stage + 1).slice(1).map(function (e) {
				return --e;
			});
		}
	...
	tittums: function (stage) {
		if (stage % 2 === 1) {
			return PlaceNotation.tittums(stage + 1).slice(1).map(function (e) {
				return --e;
			});
		}
		var row = new Array(stage), i = stage, mid = Math.floor(stage / 2);
		while (i--) {
			row[i] = Math.ceil(i / 2) + ((i % 2 === 0) ? 0 : mid);
		}
		return row;
	},
	whittingtons: function (stage) {
		if (stage < 6) {
			throw new Error('Stage must be >= 6 for Whittingtons notation.');
		}
		if (stage % 2 === 1) {
			return PlaceNotation.whittingtons(stage + 1).slice(1).map(function (e) {
				return --e;
			});
		}
		var row = new Array(stage), i = stage, mid = Math.floor(stage / 2);
